...
export function NavbarDateRangePicker() {
    ...
    const [pickHint, setPickHint] = useState('start');
    ...
    useEffect(() => {
        ...
        if (filter.mode === 'range') {
            ...
            const d = new Date(parseInt(lo.slice(0, 4), 10), parseInt(lo.slice(5, 7), 10) - 1, parseInt(lo.slice(8, 10), 10));
    ...
    const triggerLabel = filter.mode === 'all'
        ? t.posDateRangeAll
    ...
    const applyPreset = (f) => {
        ...
        const d = new Date(parseInt(f.from.slice(0, 4), 10), parseInt(f.from.slice(5, 7), 10) - 1, parseInt(f.from.slice(8, 10), 10));
        setViewYear(d.getFullYear());
        setViewMonth(d.getMonth());
        setPickHint('end');
    };
    ...
    return (_jsxs("div", { className: "relative shrink-0", ref: ref, children: [_jsxs("div", { className: "flex items-center gap-0.5", children: [_jsxs("button", { type: "button", onClick: () => setOpen((v) => !v), title: triggerLabel, "aria-label": triggerLabel, className: cn('flex h-9 shrink-0 items-center gap-1.5 rounded-xl border border-indigo-200 bg-indigo-50/80 px-2.5 text-xs font-semibold text-indigo-800 transition-colors hover:border-indigo-300 hover:bg-indigo-50 sm:max-w-[14rem] sm:bg-white sm:px-2.5 sm:text-sm sm:text-slate-600 dark:border-indigo-500/40 dark:bg-indigo-950/40 dark:text-indigo-200 dark:hover:border-indigo-400 sm:dark:border-slate-700 sm:dark:bg-slate-800 sm:dark:text-slate-300', filter.mode === 'range' ? 'min-w-[7.5rem] max-w-[9.5rem] sm:min-w-0' : 'min-w-[6.5rem] sm:min-w-0'), children: [_jsx(CalendarIcon, { size: 16, className: "shrink-0 text-indigo-600 dark:text-indigo-400" }), _jsx("span", { className: "truncate sm:max-w-[10rem]", children: triggerLabel })] }), filter.mode === 'range' && (_jsx("button", { type: "button", onClick: clearRange, title: t.navDateClearAria, className: "flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-red-200 bg-red-50 text-red-600 transition-colors hover:bg-red-100 dark:border-red-900/50 dark:bg-red-950/40 dark:text-red-400", children: _jsx(X, { size: 14 }) }))] }), open && (_jsxs(_Fragment, { children: [_jsx("button", { type: "button", className: "fixed inset-0 z-[84] bg-slate-900/30 sm:hidden", "aria-label": t.cancel, onClick: () => setOpen(false) }), _jsxs("div", { className: cn('fixed inset-x-0 bottom-0 z-[85] flex max-h-[min(92dvh,36rem)] flex-col overflow-hidden rounded-t-2xl border border-slate-200 bg-white shadow-2xl shadow-slate-300/40 dark:border-slate-700 dark:bg-slate-900 dark:shadow-black/50', 'sm:absolute sm:inset-x-auto sm:bottom-auto sm:right-0 sm:left-auto sm:top-full sm:mt-2 sm:max-h-[min(32rem,calc(100dvh-6rem))] sm:w-[22rem] sm:rounded-2xl'), children: [_jsxs("div", { className: "min-h-0 flex-1 overflow-y-auto overscroll-contain p-3 pb-2", children: [_jsxs("div", { className: "flex flex-wrap gap-1.5", children: [_jsx("button", { type: "button", onClick: presetToday, className: "rounded-full border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800", children: t.posDateRangeToday }), _jsx("button", { type: "button", onClick: presetWeek, className: "rounded-full border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800", children: t.navDateThisWeek }), _jsx("button", { type: "button", onClick: presetMonth, className: "rounded-full border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800", children: t.posDateRangeMonth }), _jsx("button", { type: "button", onClick: () => applyPreset({ mode: 'all' }), className: "rounded-full border border-slate-200 px-2.5 py-1 text-xs font-medium text-slate-600 hover:bg-slate-50 dark:border-slate-600 dark:text-slate-300 dark:hover:bg-slate-800", children: t.posDateRangeAll })] }), _jsxs("div", { className: "mt-3 grid grid-cols-2 gap-2", children: [_jsxs("div", { children: [_jsx("p", { className: "mb-1 text-[10px] font-medium uppercase tracking-wide text-slate-400", children: t.navDateFrom }), _jsx(Input, { value: fromInput, onChange: (e) => setFromInput(e.target.value), onFocus: () => {
                                                            setActiveField('from');
                                                            setPickHint('start');
