...
export function matchDemoUser(identifier: string, password: string): DemoStoredUser | null {
  ...
  return (
    readRaw().find((u) => u.login.trim() === id && u.password === password && u.isActive) ?? null
  );
}
...
export function updateDemoUser(
  ...
  if (patch.login !== undefined) next.login = patch.login.trim();
...
export function getBuiltinDemoCredentials(): { login: string; password: string } {
  try {
    ...
    const login =
      typeof p.login === 'string' && p.login.trim().length >= 2 ? p.login.trim() : 'admin';
    const password =
