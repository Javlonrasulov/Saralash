...
export type NavDateFilter = { mode: 'all' } | { mode: 'range'; from: string; to: string };

const YMD = /^\d{4}-\d{2}-\d{2}$/;

export function normalizeYmd(raw: string): string {
...
export function parseStoredNavFilter(raw: unknown): NavDateFilter {
...
export function clampYmdToMax(ymd: string, max: string): string {
  return ymd > max ? max : ymd;
}

/** Oralikni bugungi sanaga qadar qisqartiradi (kelajak yo‘q). */
export function clampFilterToToday(f: NavDateFilter): NavDateFilter {
  const max = todayYmd();
