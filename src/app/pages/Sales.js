...
import { ShoppingCart, Users, History, Trash2, Pencil, CheckCircle2, ShoppingBag, Banknote, CreditCard, } from 'lucide-react';
import { toast } from 'sonner';
import { useStore, getAvailableKgForWarehouseSale, resolveOrderPaymentMethod, } from '../store/saralash-store';
...
export function Sales() {
    ...
    const periodSoldStats = useMemo(() => {
        ...
        for (const lines of orderMap.values()) {
            ...
            const paid = first.orderPaidTotal != null && Number.isFinite(first.orderPaidTotal)
                ? Math.max(0, Math.min(first.orderPaidTotal, orderTotal))
                : orderTotal;
            totalSales += orderTotal;
            if (resolveOrderPaymentMethod(first) === 'CARD')
                cardPaid += paid;
            else
                cashPaid += paid;
