...
export function parseQtyEntry(raw) {
...
export function lineQtyParts(line) {
    return line.qtyParts ?? [];
}
/** Yig‘ilgan qismlar + ixtiyoriy kutilayotgan kiritish. */
export function lineQtyTotal(line, includePending = false) {
...
export function formatQtyPartsExpression(parts, unit) {
    if (!parts.length)
        return '';
    return parts.map((n) => formatQuantity(n, unit)).join('+');
}
export function appendQtyPart(line, unit) {
